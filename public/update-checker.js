...
class UpdateChecker {
  ...
  async loadVersionData() {
    try {
      const response = await fetch(this.versionJsonUrl, { cache: 'no-store' });
      if (!response.ok) {
  ...
  async updatePageElements() {
    ...
    if (dateElement && data.releaseDate) {
    ...
    if (changelogElement && Array.isArray(data.changelog)) {
      changelogElement.innerHTML = data.changelog.map((item) => `<li>${item}</li>`).join('');
    }

    // Atualizar botão de download
    const downloadButton = document.getElementById('download-button');
    if (downloadButton && data.downloadUrl) {
